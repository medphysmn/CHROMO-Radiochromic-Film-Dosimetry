...
for i, scan_filepath in enumerate(treatment_list):  
    ...
    plotRecalibratedImages(x_max_lsmodel, unexposedTreatmentObjects[i], maxDoseTreatmentObjects[i], recalibrated_multichannel_response_curve_red, recalibrated_multichannel_response_curve_green, recalibrated_multichannel_response_curve_blue, y_calibration_fit, i)
      
    ##############################################################################################  np.random.rand(220,220)#
    dose_red_nonfiltered =  inverse_recalibrated_multichannel_response_curve_red(netImage[:,:,redChannel])
    dose_red_filtered, min_red, max_red, avg_red, std_red, half_maximum_red_x, half_maximum_red_y, dose_red_center, bkgxred, bkgyred = dose_calculation_with_filters(dose_red_nonfiltered, netImage[:,:,redChannel], 'r', 0, 0, inverse_recalibrated_multichannel_response_curve_red, inverse_recalibrated_multichannel_response_curve_green, inverse_recalibrated_multichannel_response_curve_blue)
    ...
    ##############################################################################################
    dose_green_nonfiltered =  inverse_recalibrated_multichannel_response_curve_green(netImage[:,:,greenChannel])
    ...
    ##############################################################################################
    dose_blue_nonfiltered =  inverse_recalibrated_multichannel_response_curve_blue(netImage[:,:,greenChannel])
    ...
    bkgx3ch = (bkgxred + bkgxgreen + bkgxblue)/3
    bkgy3ch = (bkgyred + bkgygreen + bkgyblue)/3      
    dose_3ch_filtered, min_3ch, max_3ch, avg_3ch, std_3ch, half_maximum_3ch_x,half_maximum_3ch_y, dose_3ch_center, bkgx3ch, bkgy3ch = dose_calculation_with_filters(dose_3ch, [], '3ch', bkgx3ch, bkgy3ch, inverse_recalibrated_multichannel_response_curve_red, inverse_recalibrated_multichannel_response_curve_green, inverse_recalibrated_multichannel_response_curve_blue)
